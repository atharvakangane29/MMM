...
function renderLogin() {
  ...
  screen.innerHTML = `
    <div class="login-card">
      ...
      <div class="form-group">
        ...
        <input type="email" class="form-input" id="login-email" placeholder="analyst@utc.com" value="analyst@utc.com">
      </div>
      <div class="form-group">
  ...
  document.getElementById('login-btn').addEventListener('click', async () => {
    ...
    try {
      ...
      document.getElementById('nav-avatar').textContent = data.user.name.split(' ').map(n => n[0]).join('');
      ...
      btn.disabled = false;
      errBanner.classList.add('show');
      errBanner.textContent = err.message || 'Invalid email or password.';
      screen.querySelector('.login-card').style.animation = 'shake 0.4s ease';
    }
  });
...
function initDataSource() {
  ...
  const catalogSel = document.getElementById('ds-catalog');
  const schemaSel = document.getElementById('ds-schema');
  const tableSel = document.getElementById('ds-table');

  // ── Load catalogs from API ──────────────────────────────────────────────
  async function loadCatalogs() {
    try {
      const data = await api.getCatalogs();
      catalogSel.innerHTML = '<option value="">Select catalog…</option>' +
        data.catalogs.map(c => `<option value="${c}">${c}</option>`).join('');
    } catch (_) {
      // fallback to MOCK
      catalogSel.innerHTML = '<option value="">Select catalog…</option>' +
        MOCK.catalogs.map(c => `<option value="${c}">${c}</option>`).join('');
    }
  }
  ...
  async function loadSchemas(catalog) {
    ...
    try {
      const data = await api.getSchemas(catalog);
      schemaSel.innerHTML = '<option value="">Select schema…</option>' +
        data.schemas.map(s => `<option value="${s}">${s}</option>`).join('');
    } catch (_) {
      const fallback = MOCK.schemas[catalog] || [];
      schemaSel.innerHTML = '<option value="">Select schema…</option>' +
        fallback.map(s => `<option value="${s}">${s}</option>`).join('');
    }
  }
  ...
  async function loadTables(catalog, schema) {
    ...
    try {
      const data = await api.getTables(catalog, schema);
      tableSel.innerHTML = '<option value="">Select table…</option>' +
        data.tables.map(t => `<option value="${t.name}">${t.name}${t.row_count ? ' (' + t.row_count + ' rows)' : ''}</option>`).join('');
    } catch (_) {
      const fallback = MOCK.tables[schema] || [];
      tableSel.innerHTML = '<option value="">Select table…</option>' +
        fallback.map(t => `<option value="${t.name}">${t.name} (${t.row_count} rows)</option>`).join('');
    }
  }
  ...
  catalogSel?.addEventListener('change', () => {
    if (catalogSel.value) loadSchemas(catalogSel.value);
  });

  schemaSel?.addEventListener('change', () => {
    if (catalogSel.value && schemaSel.value) loadTables(catalogSel.value, schemaSel.value);
  });

  // ── Validate & Preview ─────────────────────────────────────────────────
  validateBtn?.addEventListener('click', async () => {
    const catalog = catalogSel.value;
    const schema = schemaSel.value;
    const table = tableSel.value;
    if (!catalog || !schema || !table) { toast.show('Please select a catalog, schema and table', 'info'); return; }
    ...
    try {
      const [valResult, previewResult] = await Promise.allSettled([
        api.validateTable(catalog, schema, table),
        api.getTablePreview(catalog, schema, table, 5),
      ]);

      validateBtn.innerHTML = 'Validate & Preview';
      validateBtn.disabled = false;

      const vd = valResult.status === 'fulfilled' ? valResult.value : null;
      ...
      // Preview rows
      const rows = previewResult.status === 'fulfilled' ? previewResult.value.rows : MOCK.previewRows;
      if (rows && rows.length > 0) {
        const cols = Object.keys(rows[0]);
        const selectedTable = tableSel.options[tableSel.selectedIndex];
        document.getElementById('ds-preview-meta').textContent = `${rows.length} rows shown · ${cols.length} columns`;
        document.getElementById('ds-preview-content').innerHTML = `
          <div class="preview-table-wrap">
            <table class="preview-table">
              <thead><tr>${cols.slice(0, 7).map(c => `<th>${c}</th>`).join('')}<th>…</th></tr></thead>
              <tbody>
                ${rows.map(row => `<tr>${cols.slice(0, 7).map(c => `<td>${row[c] !== null ? row[c] : '<span style="color:var(--oatmeal)">null</span>'}</td>`).join('')}<td style="color:var(--oatmeal)">…</td></tr>`).join('')}
              </tbody>
            </table>
          </div>`;
      }
  ...
  continueBtn?.addEventListener('click', async () => {
    // Fetch real data report for the selected table before proceeding
    if (appState.connection?.catalog) {
      try {
        const { catalog, schema, table } = appState.connection;
        appState.dataReport = await api.getDataReport(catalog, schema, table);
      } catch (_) {
        appState.dataReport = null; // will fall back to MOCK in renderDataReport
      }
    }
    appState.setStep(3);
    stepper.update(3);
...
function initConfigure() {
  ...
  document.getElementById('cfg-run-btn')?.addEventListener('click', async () => {
    const runBtn = document.getElementById('cfg-run-btn');
    const name = document.getElementById('cfg-name').value;
    const product = document.querySelector('#cfg-product .selected').dataset.val;
    const level = document.querySelector('#cfg-level .selected').dataset.val;
    const segment = document.querySelector('#cfg-segment .selected').dataset.val;
    ...
    runBtn.innerHTML = '<span style="display:inline-block;width:14px;height:14px;border:2px solid rgba(27,38,50,0.2);border-top-color:var(--truffle);border-radius:50%;animation:spin 0.6s linear infinite"></span> Submitting…';
    ...
    try {
      ...
      appState.setStep(5);
      stepper.update(5);
      runBtn.innerHTML = '▶ Run Attribution Model';
      runBtn.disabled = false;

      // Start real polling
      const poller = api.pollStatus(resp.scenario_id, {
        intervalMs: 5000,
        onUpdate(statusData) {
          const sc = appState.scenarios.get(resp.scenario_id);
          if (sc) {
            sc.status = statusData.status;
            sc.progress = statusData.progress_pct;
            sidebar.render();
            // Update running drawer
            const drawerProgress = document.getElementById('drawer-progress');
            const drawerMsg = document.getElementById('drawer-message');
            if (drawerProgress) drawerProgress.style.width = statusData.progress_pct + '%';
            if (drawerMsg) drawerMsg.textContent = statusData.message;
          }
        },
        async onComplete(scenarioId) {
          const sc = appState.scenarios.get(scenarioId);
          if (sc) {
            sc.status = 'SUCCESS';
            sc.completed = new Date().toLocaleString('en-US', { month: 'short', day: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' });
            sc.progress = 100;
            // Fetch and cache results
            try {
              const results = await api.getScenarioResults(scenarioId);
              appState.resultsCache = appState.resultsCache || {};
              appState.resultsCache[scenarioId] = results;
            } catch (_) { }
            sidebar.render();
          }
          toast.show(`"${sc?.name || 'Scenario'}" is ready! Click to view results.`, 'success');
        },
        onError(msg) {
          const sc = appState.scenarios.get(resp.scenario_id);
          if (sc) { sc.status = 'FAILED'; sidebar.render(); }
          toast.show('Job failed: ' + msg, 'error');
        },
      });

      appState.pollingTimers.set(resp.scenario_id, poller);
    } catch (err) {
      runBtn.innerHTML = '▶ Run Attribution Model';
      runBtn.disabled = false;
      toast.show('Failed to submit scenario: ' + err.message, 'error');
    }
  });
}
